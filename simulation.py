...
def simulate(detector_material: str, solids: list[Solid], beam: Beam, num_particles: int, debug_mode=False, full_output=False) -> NDArray:
	...
	if type(beam.energy) is Spectrum:
		savetxt(f"{run_dir}/input_spectrum.txt", stack([beam.energy.energies, beam.energy.probabilities], axis=1))
		xml.SubElement(definitions, "quantity",
		...
		try:
			os.remove(f"{run_dir}/input_spectrum.txt")
	...
	xml.indent(tree)
	tree.write(f"{run_dir}/new_input.gdml", xml_declaration=True, encoding="UTF-8")

	# check if this is the same as the last run
	try:
		same_as_it_ever_was = filecmp.cmp(f"{run_dir}/new_input.gdml", f"{run_dir}/input.gdml", shallow=False)
	except FileNotFoundError:
		same_as_it_ever_was = False
	# if not...
	if not same_as_it_ever_was:
		# overwrite previus input
		shutil.move(f"{run_dir}/new_input.gdml", f"{run_dir}/input.gdml")

		# clear previus output
		try:
			os.remove(f"{run_dir}/output.dat")
		except FileNotFoundError:
			pass

		# call the executable
		print(f"Simulating {num_particles} {beam.particle_name}s...", end=" ")
		subprocess.run(["grasshopper", "input.gdml", "output"], cwd=run_dir, stdout=subprocess.DEVNULL)
		print(f"done!")

	# read the output
	try:
		output_data = genfromtxt(f"{run_dir}/output.dat", names=True, comments=None)
	except FileNotFoundError:
